...
from model.model_freeze_backbone import Network
...
if config["use_wandb"]:
    ...
    wandb.init(project = "Medical Image Deeplabv3+ use pretrained self supervised freeze backbone")
...
path_pretrain_self_supervised = "/home/asilla/duongnh/project/Analys_COCO/tmp_folder/CrossPseudo_UpdateBranch/CPS_Kaggle/self_supervised_weight/epoch_76.pth"
...
model = Network(num_class,
                pretrained_model=path_pretrain_self_supervised,
...
total_iteration = number_iter_per_epoch * NUM_EPOCH
lr_policy = WarmUpPolyLR(base_lr, config.lr_power, total_iteration, NUM_EPOCH * config.warm_up_epoch)
...
path_save = "medical_weight/best_freeze_self_supervised.pth"
for epoch in range(s_epoch, NUM_EPOCH):
    ...
    for idx in pbar:
        ...
        current_idx = epoch * NUM_EPOCH + idx
