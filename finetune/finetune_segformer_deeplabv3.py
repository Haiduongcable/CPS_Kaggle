...
# from model import Network
from model_segformer_deeplabv3 import Network
...
wandb.init(project = "Cross Pseudo Label Segformer vs Deeplabv3+ Finetune")
...
# optimizer_l = torch.optim.AdamW(params_list_l, lr = base_lr,)
...
#                                 base_lr)        # head lr * 10
optimizer_r = torch.optim.SGD(network.branch2.parameters(),
                            lr=base_lr,
                            momentum=config.momentum,
                            weight_decay=config.weight_decay)
# config lr policy
...
model , optimizer_l, optimizer_r, s_epoch = \
    load_checkpoint("/home/asilla/duongnh/project/Analys_COCO/tmp_folder/CPS_Kaggle/weights/checkpoint_epoch_14.pth", network, optimizer_l, optimizer_r, s_epoch)
model.train()
print('begin train')
...
for epoch in range(s_epoch, config.nepochs):
    ...
    wandb.log({"Supervised Training Loss Deeplabv3+":  sum_loss_sup / len(pbar)})
    wandb.log({"Supervised Training Loss Segformer":  sum_loss_sup_r / len(pbar)})
    wandb.log({"Supervised Training Loss CPS":  sum_cps / len(pbar)})
    wandb.log({"mIOU val deeplabv3": m_IOU_deeplabv3})
    wandb.log({"mIOU val segformer": m_IOU_segformer})
