...
if config.use_wandb:
    ...
    wandb.init(project = "Cross Pseudo Label Deeplabv3+ Resnet101")
...
path_checkpoint = "weights/deeplabv3_resnet101_last.pth"
best_mIOU = 0
for epoch in range(s_epoch, config.nepochs):
    ...
    print("mDice deeplabv3 bracnh 1", mDice_deeplabv3_1)
    ...
    if average_mIOU > best_mIOU:
        ...
        save_bestcheckpoint(model, optimizer_l, optimizer_r, epoch, "weights/best_deeplab_resnet101.pth")
        
    
    save_bestcheckpoint(model, optimizer_l, optimizer_r, epoch, path_checkpoint)
