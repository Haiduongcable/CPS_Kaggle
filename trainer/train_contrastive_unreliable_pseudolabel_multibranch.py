...
from model.model_segformer_deeplabv3_representation_contrastive import NetworkSegformerDeeplabRepresentation
...
if config.use_wandb:
    ...
    wandb.init(project = "Cross Pseudo Label Deeplabv3+ ResNet 101 Vs Segformer B2 Unreliable PseudoLabel Contrastive learning ")
...
path_pretrained_segformer = 'pretrained/mit_b2.pth'
# define and init the model
model = NetworkSegformerDeeplabRepresentation(config.num_classes,\
                                            type_backbone_deeplabv3='resnet101',\
                                            pretrained_segformer = path_pretrained_segformer,\
                                            pretrained_deeplabv3 = config.pretrained_model)
...
path_save = "weights/last_model_contrastive_unreliable_segformer_vs_deeplabv3_resnet101.pth"
best_mIOU = 0 
for epoch in range(s_epoch, config.total_epoch):
    ...
    with torch.no_grad():
        segmentor = SegEvaluator(dataset, config.num_classes, config.image_mean,
                                 ...
                                 ["cuda"], False, None,
                                 False)
        m_IOU_deeplabv3, mDice_deeplabv3 = segmentor.run_model(model.branch2)
        m_IOU_segformer, mDice_segformer = segmentor.run_model(model.branch1)
        if m_IOU_segformer > best_mIOU:
            best_mIOU = m_IOU_segformer
            save_bestcheckpoint(model, optimizer_l, optimizer_r, "weights/best_contrastive_segformer_vs_deeplabv3_resnet101.pth")
    save_bestcheckpoint(model, optimizer_l, optimizer_r, path_save)
    
    if config.use_wandb:
        wandb.log({"mIOU Deeplabv3+ resnet101":  m_IOU_deeplabv3, "epoch": epoch})
        wandb.log({"mIOU Segformer B2":  m_IOU_segformer, "epoch": epoch})
        wandb.log({"mDice Deeplabv3+ resnet101":  mDice_deeplabv3, "epoch": epoch})
        wandb.log({"mDice Segformer B2":  mDice_segformer, "epoch": epoch})
